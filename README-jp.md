...
AIKernel.Core 0.1.1.1 は、Canonical Triadic Governance (CTG) の Core 実装面を
...
配布メモ: 0.1.1.1 更新は NuGet-only です。この更新ラインでは PyPI package を作成しません。
...
workspace です。0.1.1.1 Core package line では、この repository から Provider
package は公開しません。
...
memory mapping、Kernel 内部構造、Capability 内部実装を再実装しません。0.1.1.1 CTG Core
...
`AIKernel.Providers.MicrosoftAI` は Provider repository の更新まで 0.1.1
provider package line のまま扱います。0.1.1.1 Core 更新は Core、Hosting、
...
`py3-none-any` wheel として公開されています。0.1.1.1 CTG Core 更新では PyPI package を
...
v0.1.1.1 Core package family は、AIKernel.NET の契約パッケージ
`AIKernel.Abstractions`、`AIKernel.Dtos`、`AIKernel.Enums` v0.1.1.1 と同期しています。
