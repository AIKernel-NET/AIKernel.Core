...
また、Native Capability module 向けの OS 非依存な `IMemoryRegion` /
`IMemoryMapper` 抽象も Core に定義します。抽象は Core に置き、具体的な
...
LibTorch 2.12.0 / CUDA 13.0 向けの Windows/MSVC Native ABI 参照
Capability module です。CUDA と LibTorch runtime は Core runtime および
NuGet payload の外側に保ちます。Core の OS 非依存 memory mapping 抽象は利用できますが、
Kernel や OS 固有 mapper には依存しません。
...
AIKernel Native ABI capability bridge 向けの薄い Python binding です。

GitHub の subdirectory から直接 install し、既存の C ABI を `ctypes` で呼び出します。
Python package は OS 固有 memory mapping、Kernel 内部構造、Capability 内部実装を
再実装しません。
...
`AIKernel.Cuda.Libtorch.2.12-cuda13.0` は最初の Native ABI 参照 module であり、
小さな C API bridge 経由で LibTorch を呼び出します。CUDA / LibTorch runtime は
AIKernel.Core にも NuGet payload にも含めません。
