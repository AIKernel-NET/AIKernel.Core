...
現在の release note は [RELEASE_NOTES-ja.md](RELEASE_NOTES-ja.md) を参照してください。
...
`aikernel-net` は、Python user が各 repository を clone しなくても C# package
と同じ Canon / Council / Gate / RejectPolicy / locale 構造を確認できるよう、
最小 Monolith CTG-ROM を sample asset として同梱します。
...
sample は正典 AIKernel.NET ROM tree からコピーした data のみです。Python package
は CTG Gate logic を実行しません。
